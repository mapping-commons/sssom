...
[as per the standard rules](model.md#propagation).
...
2.  Iterate over all slots of the `Mapping` class, in the order in which they
    are [listed](Mapping.md/#slots) in the LinkML model. Exclude the `record_id`
...
3.  If the implementation support
    [extension slots](model.md#non-standard-slots) and the mapping record
